...
def separating_promtp_and_completion_from_examples(example):
    #separation by the first occurence of \nlocal
    index_separation = example['prompt'].find('\nlocal')
    if index_separation == -1:
        return {'source' : example['prompt'], 'output' : example['completion']}
    prompt = example['prompt'][:index_separation]
    completion = example['prompt'][index_separation+1:]
    return {'source' : prompt, 'output' : completion}
...
def create_prompts(examples):
    ...
    for example in examples:
        prompt_template = (
            PROMPT_STARCODER_DICT["prompt_with_input"] if 'input' in example.keys() and example["input"] != "" else PROMPT_STARCODER_DICT["prompt_without_input"]
        )
        updated_example = separating_promtp_and_completion_from_examples(example)
        source = prompt_template.format_map(updated_example)
        prompts["source"].append(source)
        prompts["target"].append(updated_example["output"])
    return prompts
...
def main():
    ...
    if training_args.do_train:
        ...
        print("Number of trainable paraemters:", model.num_parameters(only_trainable=True))
